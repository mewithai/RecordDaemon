...
근데 알렉사에게 한국말을 시켜보니 외국인이 한국말하는 듯 어색하다.  한국말은 MP3으로 만들어서 미디어 출력을 하면 된다.

MP3는 매번 생성하면 되는데 TTS에 비용이 부담스럽다면, 특정 상황에 맞는 멘트들은 Library로 만들어두고 상황에 맞게 재생시키면 된다.
 ...
 **HAOS**에서 알렉사 관련은 2개의 Add-on이 있는데, 이번엔 공식 Add-on을 이용해 보자.
...
HAOS의 Action에서 쉽게 찾을 수 있다. 아래 그림을 참고하자.

![alt text](image.png)

#### [실행2] Alexa에 MP3 출력시켜 보기

message에 Text가 들어가면 Alexa가 TTS로 읽어주고 미디어 파일이 들어가면 Play한다. 복합은 다음과 같이 SSML로 작성한다.  [SSML reference]("https://developer.amazon.com/en-US/docs/alexa/custom-skills/speech-synthesis-markup-language-ssml-reference.html")
