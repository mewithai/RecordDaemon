---
title: "화장실 워터건(스프레이건) 수리"
meta_title: "화장실 워터건 수리 방법 | RecordDaemon"
description: "화장실 청소용 워터건(스프레이건)의 물 고임 문제를 해결하는 방법. 아메리칸 스탠다드 제품 기준으로 플라스틱과 크롬도금 부품의 분해 및 조립 과정을 자세히 설명합니다."
date: 2025-12-06T00:00:00Z
image: "/images/20251206.png"
categories: ["집수리"]
author: "JSKim"
tags: ["집수리", "아메리칸스탠다드", "American Standard", "스프레이건", "워터건", "화장실청소"]
draft: false
---

> 화장실 청소할때 쓰는 워터건 아래에 물이 흔건하게 고였다. 한번 고쳐보자. 

 우리집 제품은 아메리칸 스탠다드 제품과 비슷하지만, 브랜드 제품은 아닌것 같다. (**레이저 각인**이 없다.)

- **아메리칸 스탠다드** 기준 모델명은 `FJ7060`, `M1BK56`. 
- **플라스틱**(ABS)에 **크롬**도금으로 되어 있다.
- 아메리칸스탠다드 정품도 ABS에 크롬도금이다. 

 설비하시는 분이 몇천원 아끼셨나보다. 눈에 보이지 않으면 정품을 쓰는 일이 없더라.

![alt text](image1.png)

 잠근 상태에서 한 두방울씩 물방울이 떨어진다.

<lite-youtube videoid="JSTowgyZzHE" videotitle="화장실 워터건 수리" class="video-custom"></lite-youtube>

 분해를 해보면 안쪽에 플라스틱 패킹과 맞닿는 부분에 O-Ring이 있다.

 ![alt text](./image2.png)

 `O-Ring`이 마모되어 물이 샐 확률이 높다. 이렇게 되면

#### 새걸 사던지.

-> 새것 기준 **10,000원 ~ 15,000원** 정도

  ![alt text](image4.png)

#### 고치던지...

-> 이런 O-Ring은 알리에서 쉽게 구할수 있다. **2,000원**..  아래를 클릭하면 이동

[![alt text](./image3.png)](https://s.click.aliexpress.com/e/_c3m4ff6p)

 구매하고 3일후에 배송되었다. 새로사는 것 보다 싸다.  패킹이 약간 크지만 두꺼워서 괜찮을 것 같다.

<lite-youtube videoid="Y1-V3TbYbmc" videotitle="패킹 분리 및 수리" class="video-custom"></lite-youtube>

교체를 하고 나니 물이 새지 않는다. 성공이다. 

<lite-youtube videoid="RPriELpOmxo" videotitle="화장실 워터건 수리완료" class="video-custom"></lite-youtube>

끝.

---

### 추가

 원래 이 워터건은 평소 항상 꺼져있는 상태로 있어야 한다고 한다. 밸브가 열려있는 상태이기 때문에 고압이 항상 걸려있고 주로 호수쪽에서 터져서 물난리가 난다고 한다. 최악의 경우는 화장실 문이 열린 상태에서 마루로 물이 튄후 한동안 방치되어 마루를 갈아야 하는 것 아닐까?

 ![alt text](image5.png)

 **T-Value**에서 누수를 발견하였고 패킹을 점검하거나 녹물이 나오면 교체할 예정. 할 일 늘었다. ㅠㅠ

 ---